...
/**
 * A class component can't call the `useT` hook, and this is the one screen
 * that has to survive even when the rest of the app — settings store included
 * — failed to render. It reads the device language directly rather than going
 * through the app's own language preference.
 */
function boundaryT() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 dark:bg-slate-950">
        <div className="w-full max-w-md text-center">
          ...
          <h1 className="mt-4 text-xl font-bold text-slate-900 dark:text-white">{t('nav.errorBoundary.title')}</h1>
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{t('nav.errorBoundary.body')}</p>
          <div className="mt-6 flex justify-center gap-3">
            <button
              onClick={this.reload}
              className="rounded-xl bg-brand-700 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-600"
            >
              {t('nav.errorBoundary.reload')}
            </button>
            <a
              href="/app/decks"
