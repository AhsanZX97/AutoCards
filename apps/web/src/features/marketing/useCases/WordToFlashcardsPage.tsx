...
function steps(t: Translator) {
  return [
    { step: '01', title: t('useCase.word.step.upload.title'), description: t('useCase.word.step.upload.description') },
    { step: '02', title: t('useCase.word.step.read.title'), description: t('useCase.word.step.read.description') },
    { step: '03', title: t('useCase.word.step.generate.title'), description: t('useCase.word.step.generate.description') },
  ];
}

/** Landing page for the "word to flashcards" search query. */
export function WordToFlashcardsPage() {
  const t = useT();
  return (
    <div className="mx-auto max-w-4xl px-6 py-20">
      ...
      <section className="mt-14 rounded-2xl border border-slate-200 bg-slate-50/60 p-6 dark:border-slate-800 dark:bg-slate-900/40">
        <h2 className="mb-2 font-display text-sm font-semibold text-slate-800 dark:text-slate-100">
          {t('useCase.word.honest.heading')}
        </h2>
        <p className="text-sm leading-relaxed text-slate-500 dark:text-slate-400">{t('useCase.word.honest.body')}</p>
      </section>

      <div className="mt-14 flex justify-center">
        <Link to="/demo">
          <BrandButton>{t('useCase.word.cta')}</BrandButton>
        </Link>
      </div>
    </div>
  );
