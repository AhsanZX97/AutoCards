...
function steps(t: Translator) {
  return [
    { step: '01', title: t('useCase.notes.step.upload.title'), description: t('useCase.notes.step.upload.description') },
    { step: '02', title: t('useCase.notes.step.read.title'), description: t('useCase.notes.step.read.description') },
    { step: '03', title: t('useCase.notes.step.generate.title'), description: t('useCase.notes.step.generate.description') },
  ];
}

/**
 * Landing page for the "lecture notes to flashcards" search query.
 *
 * Unlike the format-specific pages, "lecture notes" isn't one file type — the
 * copy is deliberately format-agnostic (Word, PDF, text) rather than pointing
 * at a single extractor.
 */
export function LectureNotesToFlashcardsPage() {
  const t = useT();
  return (
    <div className="mx-auto max-w-4xl px-6 py-20">
      ...
      <section className="mt-14 rounded-2xl border border-slate-200 bg-slate-50/60 p-6 dark:border-slate-800 dark:bg-slate-900/40">
        <h2 className="mb-2 font-display text-sm font-semibold text-slate-800 dark:text-slate-100">
          {t('useCase.notes.honest.heading')}
        </h2>
        <p className="text-sm leading-relaxed text-slate-500 dark:text-slate-400">{t('useCase.notes.honest.body')}</p>
      </section>

      <div className="mt-14 flex justify-center">
        <Link to="/demo">
          <BrandButton>{t('useCase.notes.cta')}</BrandButton>
        </Link>
      </div>
    </div>
  );
