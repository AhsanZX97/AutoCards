...
/**
 * /vs/quizlet — a fair comparison, not sales copy.
 *
 * Quizlet's existing content library, its game modes and its own AI tools
 * (Magic Notes, Q-Chat, gated behind Quizlet Plus) are real advantages and
 * stated as such.
 */
export function VsQuizletPage() {
  const t = useT();
  const competitorPoints = [
    t('compare.quizlet.competitor.point1'),
    t('compare.quizlet.competitor.point2'),
    t('compare.quizlet.competitor.point3'),
  ];
  const autocardsPoints = [
    t('compare.quizlet.autocards.point1'),
    t('compare.quizlet.autocards.point2'),
    t('compare.quizlet.autocards.point3'),
  ];
    ...
    <div className="mx-auto max-w-4xl px-6 py-20">
      <section className="flex flex-col items-center text-center">
        <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-cyan-500/25 px-4 py-1.5 text-xs font-semibold tracking-wide text-cyan-600 brand-tint dark:text-cyan-400">
          ...
          {t('compare.quizlet.eyebrow')}
        </div>
        <h1 className="mb-5 max-w-2xl font-display text-4xl font-bold tracking-tight text-slate-900 dark:text-white md:text-5xl">
          {t('compare.quizlet.h1')}
        </h1>
        <p className="max-w-xl text-lg leading-relaxed text-slate-500 dark:text-slate-400">{t('compare.quizlet.subtitle')}</p>
      </section>
      ...
      <section className="mt-16 grid grid-cols-1 gap-6 md:grid-cols-2">
        <div className="rounded-2xl border border-slate-100 bg-white/60 p-6 backdrop-blur-sm dark:border-slate-800 dark:bg-slate-900/60">
          <h2 className="mb-4 font-display text-sm font-semibold text-slate-800 dark:text-slate-100">
            {t('compare.quizlet.competitor.heading')}
          </h2>
      ...
      <section className="mt-10 rounded-2xl border border-slate-200 bg-slate-50/60 p-6 dark:border-slate-800 dark:bg-slate-900/40">
        <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-300">{t('compare.quizlet.verdict')}</p>
      </section>
